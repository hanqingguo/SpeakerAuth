...
@File: train.py
@Time: 2023.04.27
@Overview: Train and evaluate the SV performance
"""
...
from models.GE2E import GE2EModel, GE2ELoss, get_centroids, get_cossim
...
def train(model_path):
    ...
    if hp.model_name == 'glg':
        embedder_net = GE2EModel().to(device)
    ...
    for e in range(hp.train.epochs):
        ...
        for batch_id, (mel_db_batch, labels) in enumerate(train_loader):  # batch 是这个dataset load过程中的序号，我有566个speaker，每次load 两个，batch id: 0- 283
            ...
            iteration += 1
            # print (batch_id, hp.train.log_interval)
    ...
    # save model
    embedder_net.eval().cpu()
    save_model_filename = "final_epoch_" + str(e + 1) + ".pth"
...
def test(model_path):
    ...
    for e in range(hp.test.epochs):
        ...
        for batch_id, (mel_db_batch, _) in enumerate(test_loader):
            # print(mel_db_batch.shape)  # (4, 6, 160, 40)
